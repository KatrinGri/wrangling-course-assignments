...
```{r}
temp <- data_selfreport_tidied %>% #always count to see number of responses
  count(response)
...
data_selfreport_trial_level <- data_selfreport_trial_level %>% 
  filter(response %in% 1:7)   # Filter only values between 1 and 7

temp <- data_selfreport_trial_level %>% 
  count(exclude)
