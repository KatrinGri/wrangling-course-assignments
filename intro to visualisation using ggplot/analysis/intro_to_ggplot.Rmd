...
summary_self_report <- data_after_exclusions %>% 
  group_by(mean_self_report) %>% 
  summarize(self_mean = mean(mean_self_report),
            self_se = plotrix::std.error(mean_self_report))
...
ggplot(data = summary_self_report, 
  ...
  geom_col(fill = "#0b6623", # note that you can specify specific colors using hex codes or names
           color = "black", 
           width = 0.6) +
  geom_errorbar(aes(ymin = self_mean - self_se, 
                    ymax = self_mean + self_se), 
                width = 0.1, 
                color = "black") +
  labs(title = "Bar Plot of with Standard Errors",
       x = "Gender",
